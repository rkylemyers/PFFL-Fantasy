...
class PFFLApp {
  ...
  applyBloodDrips() {
    document.querySelectorAll('.team-panel, .trend-item').forEach(panel => {
  ...
  showPlayerModal(id, name, team, pos, score, projScore, isLive) {
    ...
    document.getElementById('modal-player-name').innerHTML = `<div style="display: flex; flex-direction: column; align-items: center; gap: 12px; margin-top: 10px;"><img src="${hsUrl}" onerror="this.onerror=null; this.src='https://sleepercdn.com/images/v2/icons/player_default.webp'" style="width: 120px; height: 120px; min-width: 120px; object-fit: cover; border-radius: 50%; border: 3px solid rgba(255,255,255,0.15); background: rgba(0,0,0,0.5); box-shadow: 0 6px 16px rgba(0,0,0,0.5);"><span style="font-size: 1.6rem; font-weight: 800; line-height: 1.1; white-space: nowrap; letter-spacing: -0.5px; text-align: center;">${name}</span></div>`;
    ...
    let breakdownHtml = `
        <div style="font-size: 0.95rem; color: #94a3b8; margin-bottom: 20px; font-weight: 500; text-align: center; margin-top: 5px;">
  ...
  populateSettings() {
    ...
    if (btnTestAuth) {
        btnTestAuth.addEventListener("click", async () => {
            ...
            try {
                const resp = await fetch('https://pffl-fantasy.vercel.app/api/mfl-proxy', {
                    method: 'POST',
